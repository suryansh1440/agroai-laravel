<x-guest-layout>
    <div class="relative h-full flex flex-col justify-center" x-data="{ step: 1 }">
        ...
        <div x-show="step === 1" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-x-4" x-transition:enter-end="opacity-100 translate-x-0">
            ...
            <form method="POST" action="{{ route('login') }}" class="space-y-5" x-data="{ isSubmitting: false }" @submit="isSubmitting = true">
                @csrf
                ...
                <div>
                    <button type="submit" :disabled="isSubmitting" class="w-full py-3.5 bg-[#4cc9f0] hover:bg-[#43b5d8] text-white rounded-xl text-base font-bold transition-all shadow-lg shadow-cyan-500/20 flex items-center justify-center gap-2">
